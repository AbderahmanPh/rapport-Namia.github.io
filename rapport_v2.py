...
reception = pd.read_excel('../tableau_project/data/produit_by_day.xlsx')
mep = pd.read_excel('../tableau_project/data/mep_rapport.xlsx')
rapprochement = pd.read_excel('../tableau_project/data/rapprochement_rapport.xlsx')
...
app.layout = html.Div([
    ...
    dcc.DatePickerRange(
        ...
        max_date_allowed=date(2022,9, 18),
...
def update_produit(tab, cleaned_data, produit_date):
    ...
    if tab == 'tab-1-example-graph':
        ...
        cols = [dict(name = 'PRODUIT', id = 'PRODUIT'),
                ...
                dict(name = 'PERCENTAGE', id = 'PERCENTAGE', type = 'numeric')   
                                            ]
        ...
        table_produit = DataTable(columns = cols,
                          ...
                          style_data_conditional=([
                            {
                                'if': {
                                    'filter_query': '{{PERCENTAGE}} = {}'.format(i),
                                    
                                },
                                'backgroundColor': 'hsl(120, 100%, 80%)'
                            } for i in dff['PERCENTAGE'].nlargest(3)
                          ]),
                          ...
                          style_data_conditional=([
                            {
                                ...
                                'backgroundColor': 'hsl(120, 100%, 80%)' #'#a6ffb9'
                            } for i in dff['PERCENTAGE'].nlargest(3)
                          ]),
